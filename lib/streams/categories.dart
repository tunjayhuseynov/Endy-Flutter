...
class CategoryStream {
  static Future<Stream<List<Category>>> getCategories() async {
    return FirebaseFirestore.instance
        .collection('categories')
        .orderBy('name')
        .snapshots()
        .map((snapshot) => snapshot.docs.map((doc) {
              ...
              category.subcategory.map((e) async {
                Subcategory category = await SubcategoryCrud.getSubcategory(
                    (e as DocumentReference).id);
                e = category;
              });
              return category;
            }).toList());
  ...
  static Stream<Category> getCategory(String id) {
    return FirebaseFirestore.instance
        .collection('categories')
        .doc(id)
        .snapshots()
